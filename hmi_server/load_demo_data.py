...
def load_ecus(version_mapping):
    ...
    for i, (ecu_id, ecu_data) in enumerate(zip(ecu_ids, ecus_data)):
        ...
        ecu_data["version_ids"] = [version_mapping[v_id]
                                   for v_id in ecu_data["version_ids"]]
...
def load_car_types(ecu_mapping):
    ...
    for car_type_data in car_types_data:
        ...
        car_type_data["ecu_ids"] = [ecu_mapping[e_id]
                                    for e_id in car_type_data["ecu_ids"]]

        # Insert into MongoDB
        result = car_types_collection.insert_one(car_type_data)
        print(
            f"  Inserted car type: {car_type_data['name']} -> {result.inserted_id}")
...
def load_sample_requests():
    ...
    for request_data in requests_data:
        ...
        print(
            f"  Inserted request for car: {request_data['car_id']} -> {result.inserted_id}")
    ...
    for request_data in download_requests_data:
        ...
        print(
            f"  Inserted download request for car: {request_data['car_id']} -> {result.inserted_id}")
...
def main():
    ...
    try:
        ...
        print(
            f"Download Requests: {download_requests_collection.count_documents({})}")
